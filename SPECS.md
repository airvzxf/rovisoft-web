...
- Hay una discrepancia entre el cambio de idioma. Si yo cambio de español a inglés, pero ya tengo varios comandos ejecutados, se actualiza el banner de inicio y el texto de bienvenida de inicio. Pero no se actualiza lo demás. Ya si escribo un nuevo comando ya está en inglés. Entonces me da curiosidad saber por qué se cambia dinámicamente la bienvenida con el banner. Y si esto no tendría que suceder en todos los comandos ya ejecutados. O al revés?
...
- Mejorar el rendimiento dividiendo el archivo main.js en más pequeños sobre todo los que se necesitan para el inicio del DOOM para que no de pena descargar los 50 kilobytes y luego cargue el DOM. Hay que evitar los flash. Esto inclusive podría incluir dividir los idiomas y que más bien se pidan cuando cuando se haga el cambio de idioma descargue el archivo o se bajen en background progresivamente porque imaginemos que tenemos 50 idiomas pues van a va a ser mucho no sé cómo se maneja esto de la mejor forma profesional o la forma industrial o la forma más chingona.
